...
public class ValidateUtils {
	...
	public void validateGet(Sprinkle sprinkle, long userId, String roomId) {
		...
		if (sprinkle.getSprinkleGets().stream()
				...
				.anyMatch(get -> get.getUserId().equals(userId))) throw new DuplicateGetException(ResponseCodes.DUPLICATE_GET);
	}
	/**
	 조회시 검증 로직
	 */
	public void validateSelect(Sprinkle sprinkle, long userId) {
		if (!sprinkle.getUserId().equals(userId)) throw new PermissionDeniedException(ResponseCodes.SELECT_PERMISSION_DENIED);
